...
`test_event_wiring.py` is worth reading even if you're not modifying this codebase — it's a regression suite that specifically encodes findings from the project's own `docs/V2_ARCHITECTURE.md` audit (e.g. `TestCascadeSubscriptionGap`, asserting `PriorityPipeline` actually receives `HighScoreLinksEvent`), which is a genuinely useful pattern: turning an architecture audit's findings directly into tests that prevent the same class of bug recurring.
