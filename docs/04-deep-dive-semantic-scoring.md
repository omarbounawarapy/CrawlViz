...
| `high` | score > `HIGH` | A top-K-plus-random sample is sent to the LLM (to spend a little budget confirming high-confidence guesses and improve precision); the rest is fast-tracked as `trusted_no_llm` — its priority is computed from NLP signals alone, per the strategy in use. |

This is the report's cascade architecture (§1.3.2) implemented directly: a fast filter that's always on, followed by a costly evaluation applied only where it changes the decision.
...
- **`aggressive`** weights raw target similarity heavily and applies a depth penalty — favors staying close to the topic and to the seed.
- **`balanced`** splits weight evenly between the LLM score and a blend of NLP signals.
- **`exploration`** weights novelty and coverage-of-underexplored-regions more heavily than raw similarity — favors breadth over tight topical focus.

`PriorityPipeline` calls the configured strategy with `nlp_bias=60, llm_bias=40` for any link that has an LLM score, or `(1, 0)` — pure NLP, no LLM contribution — for `trusted_no_llm` links that skipped the LLM stage entirely. This second case is a direct implementation of the report's claim that fast-tracked high-confidence links get their priority "computed uniquely from NLP signals depending on the strategy" (§1.3.2).
...
Not accurately, no — and the mechanism above is why the shorthand undersells the design. The LLM is a **selectively-invoked ranking signal**, not a gatekeeper: it's consulted for a budgeted, strategy-chosen sample of links, its output is one weighted input among several into a priority function it doesn't control the weighting of, and the majority of links in a typical node (the clearly-low and the clearly-high) never reach it at all. The system that decides which links to visit is the priority function and the frontier queue; the LLM's role is to supply one high-quality, expensive signal to a subset of the decisions that function makes, and to occasionally expand what "on topic" even means via conceptual expansions. That's a meaningfully different (and more defensible, cost-wise) architecture than "the LLM picks the links," and it's worth describing that way rather than compressing it into the shorthand.
