...
All query logic is delegated to validation_db.py. No transformation of
stored values happens here.
"""
...
def list_crawls(table: str):
    ...
    Response shape per item: ``{crawl_id: str, count: int, last_seen: str}``.
    """
    try:
...
def sample_rows(
    table: str,
    crawl_id: str | None = Query(default=None, description="Filter by crawl_id"),
    ...
    All values are returned as-is from the database; large-field
    truncation and formatting are the UI's responsibility.
    """
    try:
        validate_table_name(table)
    except ValueError as exc:
