...
def _llm(link) -> float:
    """LLM score normalised to 0-1. Defaults to 0 if absent."""
...
def aggressive(node, link , nlp_bias , llm_bias) -> float:
    ...
    """
    nlp  = ( 0.40 * _nlp(link, "target_similarity")
...
def balanced(node, link, nlp_bias=0.5, llm_bias=0.5):
    nlp = (
    ...
    return round(
        max(0.0, nlp_bias * nlp + llm_bias * _llm(link)),
        6
    )
...
def exploration(node, link, nlp_bias=0.85, llm_bias=0.15):
    nlp = (
...
StrategyFn = Callable[[object, object], float]
...
def get_strategy(name: str) -> StrategyFn:
    ...
    Raises ValueError with a clear message for unknown names.
    """
...
def register_strategy(name: str, fn: StrategyFn) -> None:
    ...
    fn must satisfy: fn(node, link) -> float
    """
