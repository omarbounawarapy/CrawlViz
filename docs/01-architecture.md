...
**Reading this diagram:** the control plane exists independently of any running crawl — you can list templates or check `/status` with nothing running. The data plane is instantiated fresh per crawl by `core.Crawler` (built in `core/crawler.py`, bootstrapped by `core/boot_strapper.py`). Every pipeline talks to the broker and nothing else; there are no direct pipeline-to-pipeline method calls in the crawl's steady-state operation; this is what "event-driven" concretely means here, not a loose synonym for "asynchronous."
...
`main.py`'s three routers (`templates`, `run`, `validation`) are always live, independent of whether a crawl is executing — this is explicitly framed in the codebase as CQRS-adjacent: commands (`POST /run`, `POST /stop`) go through `routes/run.py` and mutate the running `Crawler` instance; queries about a *finished* crawl's data go through `routes/validation.py` and read `items.db` directly, never touching the live in-memory `Storage`. The live in-memory graph, meanwhile, is exposed only through the WebSocket data plane (`UIWebSocketGateway`), not through REST. A REST client cannot ask "what does the graph look like right now" — only a WebSocket client, subscribed to the live event stream, can.
...
The report and the codebase both distinguish two persistence concepts that are easy to conflate:
