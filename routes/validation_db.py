...
# We keep this strict so no user-supplied string can escape the identifier slot.
...
# Columns that every crawler table is guaranteed to have (from exporting_pipeline.py)
_REQUIRED_CRAWLER_COLUMNS = frozenset({"id", "crawl_id", "url", "created_at"})


def validate_table_name(table: str) -> str:
    """
    Raise ValueError if `table` is not a safe, recognised crawler table name.
    Returns the validated name unchanged so callers can use it directly.
    """
...
def get_connection() -> Generator[sqlite3.Connection, None, None]:
    """
    Yield a short-lived, read-optimised SQLite connection.
    WAL mode ensures reads do not block concurrent writes.
    Connection is always closed on exit.
    """
    conn = sqlite3.connect(f"file:{DB_PATH}?mode=ro", uri=True, check_same_thread=False)
    conn.row_factory = sqlite3.Row
    try:
        # WAL is a database-level setting; setting it here is a no-op if already
        # enabled by the writer, but harmless and explicit.
...
def fetch_crawls_for_table(table: str) -> list[dict]:
    ...
    Each record: { crawl_id, count, last_seen }
    """
...
def fetch_sample(
    ...
    Parameters
    ----------
    table    : validated crawler table name
    crawl_id : optional filter; if None all crawl_ids are included
